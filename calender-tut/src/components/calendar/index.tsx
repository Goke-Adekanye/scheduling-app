...
const CalenderPicker: FC<CalenderPickerProps> = ({
  ...
  useEffect(() => {
    if (selectedValue?.day) {
      const fullDayFormat =
        selectedValue.day.toString().length < 2
          ? `0${selectedValue.day}`
          : selectedValue.day;
      const fullMonthFormat =
        selectedValue.month.toString().length < 2
          ? `0${selectedValue.month}`
          : selectedValue.month;
  ...
  const getStartWeekDay = (_activeMonth: number, _activeYear: number) => {
    if (_activeMonth > 9) {
      const dateReview =
        new Date(_activeYear + "-" + _activeMonth + "-" + "01").getDay() + 1;
  ...
  const checkDisabled = (value: number) => {
    ...
    if (
      activeYear === disable_data_mock.year &&
      activeMonth === disable_data_mock.month
    ) {
  ...
  const getIsSelected = (value: number) => {
    if (
      selectedValue?.month === activeMonth &&
      selectedValue?.year === activeYear
    ) {
  ...
  const getDateInfo = () => {
    ...
    returnValue.push(
      weekShortArray.map((item, key) => (
        <div key={key} className="week-title">
          {item}
        </div>
      ))
    );
      ...
      if (i <= startWeekDay) {
        ...
        tempArray.push(
          <div
            ...
            className={`weekDays 
                        ${
                          getIsToday(tempHolder) && getIsSelected(tempHolder)
                            ? "active change-dot"
                            : getIsToday(tempHolder)
                            ? "active"
                            : ""
                        }
                        ${getIsSelected(tempHolder) ? "selected" : ""}
                        ${
                          checkDisabled(tempHolder)
                            ? "disabled"
                            : "available-days"
                        }`}
          >
  ...
  const handleMonthChange = (value: number) => {
    ...
    // console.log(changeInValue, activeMonth);

    if (
      changeInValue < getToday().getMonth() + 1 &&
      activeYear === getToday().getFullYear()
    )
      return;
    else if (changeInValue === 13) {
      setActiveMonth(1);
  ...
  const handleYearChange = (value: number) => {
    ...
    if (changeInValue < getToday().getFullYear()) return;
